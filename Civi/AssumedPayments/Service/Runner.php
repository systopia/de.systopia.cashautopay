...
namespace Civi\AssumedPayments\Service;
...
final class Runner {
      ...
      try {
        $contrib = Contribution::create(FALSE)
          ->setValues([
            ...
            'source'                 => 'assumed_payments:' . $item['period_key'],
          ])
          ->execute()
          ->first();
  ...
  // phpcs:enable
    ...
    $ids = $idsParam !== NULL
      ? $this->normalizeIds($idsParam)
      : (array) $settings->get('assumed_payments_payment_instruments');

    $dateTo = isset($params['date_to']) && $params['date_to']
      ? new \DateTime($params['date_to'])
      : new \DateTime('now');
    $grace = (int) ($settings->get('assumed_payments_grace_days') ?? 0);
    if ($grace > 0) {
      $dateTo->modify('-' . $grace . ' days');
    }
    ...
    foreach ($recurList as $r) {
      ...
      while ($cursor <= $dateTo) {
        ...
        $exists = Contribution::get(FALSE)
          ...
          ->addWhere('source', '=', 'assumed_payments:' . $periodKey)
          ->setLimit(1)
          ->execute()
          ->first();
        ...
        if ($limit !== NULL && count($toCreate) >= $limit) {
          break 2;
        }
      }
    }

    $maxCatch = (int) ($settings->get('assumed_payments_max_catchup_cycles') ?? 0);
    if ($maxCatch > 0) {
      $grouped = [];
      foreach ($toCreate as $t) {
        $grouped[$t['recur_id']][] = $t;
      }
      $limited = [];
      foreach ($grouped as $items) {
        $limited = array_merge($limited, array_slice($items, 0, $maxCatch));
      }
      $toCreate = $limited;
    }
  ...
  private function addInterval(\DateTime $d, $unit, $n) {
    $d2 = clone $d;
    if ($unit === 'month') {
      return $d2->modify('+' . $n . ' month');
    }
    if ($unit === 'year') {
      return $d2->modify('+' . $n . ' year');
    }
    if ($unit === 'week') {
      return $d2->modify('+' . $n . ' week');
    }
  ...
  private function normalizeIds($v) {
    if (is_array($v)) {
      return array_values(array_filter(array_map('intval', $v), function($x) {
        return $x !== 0 || $x === 0;
      }));
    }
    if (is_scalar($v)) {
